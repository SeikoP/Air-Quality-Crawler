...
# Configuration
BASE_DIR = Path(os.getenv('BASE_DIR', r'D:\Project_Dp-15'))
DATA_DIR = BASE_DIR / 'Source' / 'data' / '2025-06'
CLEANED_DIR = BASE_DIR / 'Source' / 'data_cleaned'
TRANSFORM_DIR = BASE_DIR / 'Source' / 'data_tranform'
...
def setup_directories():
    ...
    for directory in [CLEANED_DIR, TRANSFORM_DIR]:
        directory.mkdir(parents=True, exist_ok=True)

def load_data() -> pd.DataFrame:
    """Load and concatenate CSV files from the data directory."""
    try:
        csv_files = [f for f in DATA_DIR.glob('*.csv') if 'all' not in f.name]
        if not csv_files:
            raise FileNotFoundError(f"No CSV files found in {DATA_DIR}")
        df = pd.concat([pd.read_csv(f) for f in csv_files], ignore_index=True)
        logger.info(f"Loaded {len(df)} records from {len(csv_files)} files")
        return df
...
def save_to_csv(df: pd.DataFrame, mapping: Dict[str, pd.DataFrame]):
    ...
    try:
        ...
        for table_name, table_data in mapping.items():
            file_path = TRANSFORM_DIR / f"{table_name}.csv"
